	...
	<article v-else :class="$style.article" :style="{ cursor: expandOnNoteClick ? 'pointer' : '', paddingTop: defaultStore.state.showSubNoteFooterButton && appearNote.reply && !renoteCollapsed ? '14px' : '' }" @click.stop="noteClick" @dblclick.stop="noteDblClick" @contextmenu.stop="onContextmenu">
		...
		<div>
			...
			<footer :class="$style.footer">
				...
				<button v-if="appearNote.myReaction == null && defaultStore.state.showQuickHeartReaction !== 'reactionOnly' && defaultStore.state.showQuickHeartReaction !== 'hideAll' || appearNote.myReaction == null && appearNote.reactionAcceptance === 'likeOnly' && defaultStore.state.showQuickHeartReaction !== 'hideAll'" ref="heartReactButton" v-vibrate="defaultStore.state.vibrateSystem ? [30, 50, 50] : []" v-tooltip="i18n.ts.like" :class="$style.footerButton" class="_button" @click.stop="heartReact()">
					<i class="ti ti-heart"></i>
				</button>
				<button v-if="appearNote.reactionAcceptance !== 'likeOnly' && defaultStore.state.showQuickHeartReaction !== 'hideAll' && defaultStore.state.showQuickHeartReaction !== 'heartOnly'" ref="reactButton" v-vibrate="defaultStore.state.vibrateSystem ? [30, 50, 50] : []" :class="$style.footerButton" class="_button" @click.stop="react()">
				...
				<button v-if="appearNote.myReaction != null && appearNote.reactionAcceptance == 'likeOnly' && defaultStore.state.showQuickHeartReaction !== 'hideAll'" ref="reactButton" v-vibrate="defaultStore.state.vibrateSystem ? [30, 50, 50] : []" v-tooltip="i18n.ts.removeReaction" :class="[$style.footerButton, $style.reacted]" class="_button" @click.stop="undoReact(appearNote)">
