...
- 클라이언트: 핀 고정 사용자 설정 항목이 없는 문제를 수정
...
- CherryPick 명령줄 옵션 폐지
  - 대신 환경변수로 설정할 수 있습니다
- 페이지 로드 오류 페이지에 새로 고침 버튼 추가

### Bugfixes
- UI(friendly): 헤더에 타이틀이 표시되지 않는 문제 수정
- 클라이언트: 헤더의 탭이 반환되는 문제 수정
- 클라이언트: 헤더에 탭이 표시된 상태에서 타이틀을 클릭했을 때 탭 선택 팝업이 표시되는 문제 수정
- 클라이언트(friendly): 안테나, 리스트 타임라인을 불러올 수 없는 문제 수정
- 클라이언트: 유저 페이지의 탭이 작동하지 않는 문제 수정
