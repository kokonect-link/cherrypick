...
- Feat: 타임라인 성능 최적화
  - 화면에서 표시되지 않는 노트 또는 알림을 DOM에서 제거해 성능을 향상시키고 메모리 사용률을 감소합니다.
  - 기본값인 `20` 기준으로, 스크롤 해 불러온 노트 또는 알림이 20개 이하인 경우에는 타임라인 성능 최적화를 하지 않습니다.
		20개를 초과하는 경우에는 사용자 경험을 위해 화면에 보이는 노트 또는 알림을 기준으로 위 아래 약 10개의 항목을 DOM에 유지하며, 이후에는 DOM에서 제거됩니다.
